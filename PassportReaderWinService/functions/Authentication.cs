...
namespace tutorial
{
    ...
    class MainClass
    {
        ...
        public int Program()
        {
            ...
            //*******************************************************************************************************************
            // ** check the reader and the cards
            //*******************************************************************************************************************
            ...
            //first page
            {
                ...
                //*******************************************************************************************************************
                // ** to generate the original image with out crop and flip
                //*******************************************************************************************************************
                ...
                try
                {
                    ...
                    VizReadingTask.Add(FieldSource.All, FieldId.All);
                    Document MrzDoc = OcrEngine.Analyze(page1, VizReadingTask);
                    System.Collections.Generic.List<FieldReference> Fields = MrzDoc.GetFields();
                    //string abd = Convert.ToBase64String(Scanner.GetPage(0).Select(Pr22.Imaging.Light.White).DocView().GetImage().Save(Pr22.Imaging.RawImage.FileFormat.Jpeg).ToByteArray());
                    Scanner.GetPage(0).Select(Pr22.Imaging.Light.White).DocView().GetImage().Save(Pr22.Imaging.RawImage.FileFormat.Jpeg).Save("Abd.jpeg");

                    //Field CurrentField = MrzDoc.GetField(Fields[0]);
                    //System.Console.WriteLine(Fields[0].Id.ToString());
                    //Field CurrentField2 = MrzDoc.GetField(Fields[2]);
                    //System.Console.WriteLine(Fields[2].Id.ToString());
                    //System.Console.WriteLine(CurrentField.GetBasicStringValue());
                    //System.Console.WriteLine(CurrentField2.GetBestStringValue());
                    //CurrentField.GetImage().Save(Pr22.Imaging.RawImage.FileFormat.Jpeg).Save("face.jpeg");
